...
- 设置页面
  ...
  - 紧凑布局
  - 字体大小
  - 清除本地缓存（重置插件设置与常用网站，不删除 Chrome 书签）
  - 关于版本信息
- 同步与存储
  - `browser.storage.sync`：设置项 + 常用网站
  - `browser.storage.local`：最近打开记录
- 新标签页接管
...
- 目前未引入自动化 E2E（仅完成构建与类型验证），建议后续补充 Playwright 关键路径回归。
